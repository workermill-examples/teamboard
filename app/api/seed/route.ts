...
/**
 * POST /api/seed - Idempotent database seeding
 * Requires Bearer SEED_TOKEN authentication
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const bugCards = [
      ...
      { title: 'Memory leak in real-time updates', column: 2, priority: 'CRITICAL', assignee: 2 },
    ...
    for (const cardInfo of bugCards) {
      const card = await prisma.card.create({
        data: {
          ...
          assigneeId: allUsers[cardInfo.assignee].id,
          priority: cardInfo.priority as any,
          coverColor: cardInfo.priority === 'CRITICAL' ? '#fee2e2' : null,
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        boards: [roadmapBoard.title, sprintBoard.title, bugBoard.title],
